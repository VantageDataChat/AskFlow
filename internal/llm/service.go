...
func (s *APILLMService) GenerateWithHistory(prompt string, context []string, history []HistoryMessage, question string) (string, error) {
	...
	// Add enhanced multi-turn conversation rules
	systemContent += "\n\n【多轮对话规则】" +
		"\n这是一个多轮对话场景。用户的当前问题可能是对之前对话的追问或补充。" +
		"\n请仔细阅读对话历史，理解用户问题的真实意图。"

	// Add strict reference material usage rules
	systemContent += "\n\n【参考资料使用规则 - 极其重要】" +
		"\n1. 识别对话主题：如果这是追问（如'详细说明'、'列出清单'、'给出详细'、'具体有哪些'），" +
		"必须先从对话历史中提取主题关键词（如'OCR'、'图片格式'、'视频处理'等）。" +
		"\n2. 严格筛选资料：参考资料可能包含多个不同主题的内容。你必须只使用与对话主题直接相关的部分，" +
		"完全忽略其他主题的内容。这是强制要求，不可违反。" +
		"\n3. 主题匹配优先：如果参考资料同时包含多个模块或工具的信息（如OCR和qmFileType），" +
		"严格选择与对话历史主题匹配的那个，绝对不要混淆。" +
		"\n\n【关键示例 - 必须遵守】" +
		"\n场景：用户先问'OCR支持哪些图片格式？'，然后追问'给出详细清单'" +
		"\n参考资料包含：[1] QmImgUtil/OCR图片格式列表  [2] qmFileType文件识别格式列表" +
		"\n✓ 正确做法：只使用[1] OCR/QmImgUtil的图片格式，完全忽略[2]" +
		"\n✗ 错误做法：使用[2]文件识别格式，或混合使用两个列表" +
		"\n\n如果参考资料中没有与对话主题相关的内容，请明确告知用户，绝对不要使用不相关的资料来回答。"

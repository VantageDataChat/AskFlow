...
func migrateProductTables(db *sql.DB) error {
	migrations := []struct {
		...
		{"products", "conversation_enabled", "ALTER TABLE products ADD COLUMN conversation_enabled INTEGER DEFAULT 0"},
	}
...
func migrateTables(db *sql.DB) error {
	...
	migrations := []struct {
		...
		{"pending_questions", "linked_faq_id", "ALTER TABLE pending_questions ADD COLUMN linked_faq_id TEXT DEFAULT ''"},
	}

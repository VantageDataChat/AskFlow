...
```
┌──────────────────────────────────────────────────────────────────────┐
│                        前端 SPA (frontend/dist)                      │
│          原生 JS · 照片墙画廊 · 媒体弹窗 · 流式视频 · i18n           │
└────────────────────────────────┬─────────────────────────────────────┘
                                 │ HTTP API (60+ 接口)
┌────────────────────────────────▼─────────────────────────────────────┐
│                     Go HTTP Server (单二进制)                         │
...
│  │  │ 自动排队 │  │ Excel·PPT·MD │  │ 文本+图片 │  │ 含视觉模型 │  │ │
│  │  │ 自动入库 │  │ 纯 Go 实现   │  │ 批量嵌入  │  │            │  │ │
│  │  └──────────┘  └──────────────┘  └──────────┘  └────────────┘  │ │
│  │                                                                  │ │
│  │  ┌──────────┐  ┌──────────────┐  ┌──────────┐  ┌────────────┐  │ │
│  │  │  Video   │  │   Chunker    │  │  Email   │  │   Backup   │  │ │
│  │  │  Parser  │  │  512 字符    │  │  SMTP    │  │ 全量+增量  │  │ │
│  │  │ ffmpeg + │  │  128 重叠    │  │  TLS     │  │ 单命令恢复 │  │ │
│  │  │RapidSpeech│ │              │  │          │  │            │  │ │
│  │  └──────────┘  └──────────────┘  └──────────┘  └────────────┘  │ │
│  └──────────────────────────────────────────────────────────────────┘ │
│                                                                      │
...
│  │     │ 字符 bigram 相似度 · 预计算索引 · 缓存回答复用             │ │
│  │     ▼                                                            │ │
│  │  Level 2: 向量确认 ──→ 仅 Embedding API                         │ │
│  │     │ Embedding 缓存 (512条, 10min TTL) · 缓存回答复用           │ │
│  │     ▼                                                            │ │
│  │  Level 3: 完整 RAG ──→ Embedding + LLM                          │ │
│  │     意图分类 → 多模态向量检索 → 上下文构建 → LLM 生成            │ │
│  │                                                                  │ │
│  └──────────────────────────────────────────────────────────────────┘ │
│                                                                      │
│  ┌─── 存储与向量计算层 ─────────────────────────────────────────────┐ │
│  │                                                                  │ │
│  │  ┌─ SQLite (WAL) ──────────┐  ┌─ 内存向量引擎 ───────────────┐ │ │
│  │  │ 文档·分块·用户·会话·    │  │ 连续内存 Arena (float32)     │ │ │
│  │  │ 产品·待处理·视频片段    │  │ Product 分区索引             │ │ │
│  │  │ 外键约束 · 参数化查询   │  │ Per-Worker Min-Heap Top-K    │ │ │
│  │  └─────────────────────────┘  │ LRU 查询缓存 (FNV-1a)       │ │ │
│  │                                │ 自适应并发 Worker            │ │ │
│  │  ┌─ SIMD 加速 (20项优化) ──┐  └──────────────────────────────┘ │ │
│  │  │ AVX-512  64 floats/iter │                                    │ │
│  │  │ AVX2+FMA 32 floats/iter │  ← 运行时 CPUID 自动选择最快路径  │ │
│  │  │ ARM NEON 16 floats/iter │                                    │ │
│  │  │ SSE / Pure Go 回退      │                                    │ │
│  │  └─────────────────────────┘                                    │ │
│  │                                                                  │ │
│  └──────────────────────────────────────────────────────────────────┘ │
│                                                                      │
│  ┌─ 安全层 ────────────────────────────────────────────────────────┐ │
│  │ AES-256-GCM 加密 · bcrypt 哈希 · OAuth state 防 CSRF           │ │
│  │ IP 限流 · 失败锁定 · 文件白名单 · CSP 安全头 · 路径遍历防护    │ │
│  └──────────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────┘
                                 │
                    ┌────────────▼────────────┐
...
| 组件 | 技术选型 |
|------|----------|
| 后端 | Go 1.25+，单二进制部署，零外部依赖 |
| 数据库 | SQLite（WAL 模式，外键约束，参数化查询防注入） |
| 向量存储 | 连续内存 Arena（float32）+ Product 分区索引 + LRU 缓存 + SIMD 加速余弦相似度 |
| SIMD 加速 | AVX-512 / AVX2+FMA / ARM NEON+FMLA / SSE，运行时 CPUID 自动分发，Plan 9 汇编实现 |
| 检索优化 | 3 级渐进式匹配（文本→向量缓存→完整 RAG），逐级递进节省 API 成本 |
| 去重机制 | 文档级 SHA-256 哈希去重 + 分块级向量复用，避免冗余 API 调用 |
| LLM | OpenAI 兼容 Chat Completion API（支持视觉模型） |
| Embedding | OpenAI 兼容 Embedding API（支持多模态：文本 + 图片） |
| 文档解析 | GoPDF2、GoWord、GoExcel、GoPPT（纯 Go 实现，零 CGO 依赖） |
| 视频处理 | ffmpeg（音频提取 + 关键帧抽取）+ RapidSpeech.cpp（本地离线 ASR） |
| 前端 | 原生 JS SPA（照片墙画廊、媒体弹窗、流式视频、i18n） |
| 认证 | OAuth 2.0（Google/Apple/Amazon/Facebook）+ bcrypt + Session |
| 加密 | AES-256-GCM（API Key 加密存储） |
| 独立模块 | sqlite-vec 独立 Go module，SIMD 向量引擎可跨项目复用 |

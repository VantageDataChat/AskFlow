...
%SSHPASS% -p %PASS% ssh -o StrictHostKeyChecking=accept-new %USER%@%SERVER% "cd %REMOTE_DIR% && rm -rf *.go internal sqlite-vec && tar -xzf deploy.tar.gz && rm -f deploy.tar.gz && go get -u github.com/VantageDataChat/GoPPT && go mod tidy && go build -o %BINARY_NAME% . 2>&1"
if %errorlevel% neq 0 (
